...
W, H = 1664, 960
BOX = (0.56, 0.22, 0.86, 0.44)  # the guide's bounding box, normalised
HARD_CAP_S = 170
STAGE_S = 12

PROMPT_WHITE = ("Turn the flat grey van-shaped silhouette at the kerb on the right, in front of the low building into a real white "
                "panel van, parked exactly there at the same size, side-on to the camera. "
                "Keep everything else unchanged: the pub, the black fence, the buildings, the trees, the road markings, the sunlight and the camera. The van stays parked and still.")
PROMPT_REF = PROMPT_WHITE.replace("a real white panel van", "the van from the reference image")
PROMPT_NAVY = ("Turn the flat grey van-shaped silhouette at the kerb on the right, in front of the low building into a real dark navy "
...
def make_source():
    ...
    img.save(os.path.join(HERE, "real_source.png"))
...
async def main():
    ...
    async def stage(name, prompt=None, ref=None):
        ...
        f = out["frame"].copy()
        Image.fromarray(f).save(os.path.join(HERE, f"real_{name}.png"))
